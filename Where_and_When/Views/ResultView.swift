...
struct ResultView: View {
    ...
    var correctLocation: MKPointAnnotation {
        return MKPointAnnotation(__coordinate: CLLocationCoordinate2D(latitude: photo.location["lat"] ?? 51, longitude: photo.location["lng"] ?? -60))
    }
    var distance: Int {
        Int(calculateDistance(correctLocation, (guessedLocation!)).rounded() / 1000)
    }
    var correctYear: Int {
        return Int((photo.dateTaken)) ?? 1970
    }
...
struct ResultView_Previews: PreviewProvider {
    static var previews: some View {
        ResultView(guessedLocation: previewMapState.guess!, isGameSummary: false, round: 5, photo: previewPhotos[0], guessedYear: 1950)
